# Define the full content of the simple_demo.py file
test_demo = '''
"""
Simple Demo of AI vs Traditional Routing
Run this to see the algorithms work without Streamlit UI
"""
...
class SimpleNetworkDemo:
    ...
    def add_congestion(self, node1, node2):
        """Add congestion to a link (makes it 3x more expensive)"""
...
def main_demo():
    ...
    if trad_path:
        print(f"🔄 Traditional Route: {' → '.join(trad_path)}")
    else:
        print("🔄 Traditional Route: No path found")
    ...
    print(f"🔄 Traditional Route (with congestion): {' → '.join(trad_path_cong)}")
    print(f"💰 Cost: {trad_cost_cong} (higher due to congestion!)")
    ...
    print(f"\\n📊 COMPARISON:")
    print(f"   Traditional: {' → '.join(trad_path_cong)} (Cost: {trad_cost_cong})")
    print(f"   AI Learning: {' → '.join(ai_path) if ai_path else 'Failed'} (Cost: {ai_cost})")
    
    if ai_cost < trad_cost_cong:
        print(f"\\n🏆 AI WINS! Saves {trad_cost_cong - ai_cost} cost units!")
        print("   AI learned to avoid congested route!")
    else:
        print(f"\\n🤔 Traditional method was better this time")
    
    print(f"\\n✨ Key Learning: AI adapts to network conditions,")
    print(f"   Traditional always uses same route regardless of congestion!")

if __name__ == "__main__":
    main_demo()
'''

# Write the file with UTF-8 encoding to avoid UnicodeEncodeError
with open('simple_demo.py', 'w', encoding='utf-8') as f:
    f.write(test_demo)

print("✅ simple_demo.py created successfully with proper UTF-8 encoding.")
